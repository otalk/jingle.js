...
FileSession.prototype = _.extend(FileSession.prototype, {
    ...
    onSessionInitiate: function (changes, cb) {
        ...
        console.log('metadata', this.receiver.metadata);

        // set hash used by peer
        if (this.receiver.metadata.hash) {
            // FIXME: remove bad hack when stanza does the right thing
            this.receiver.config.hash = this.receiver.metadata.hash.algo.replace('-', '');
        }
    ...
    maybeReceivedFile: function () {
        if (this.receiver.metadata.hash.value === this.receiver.metadata.actualhash) {
